...
void MiiShader::Init()
{
    mShader.load("mii_shader", rio::Shader::MODE_UNIFORM_REGISTER);

    mVertexUniformLocation[VERTEX_UNIFORM_VP] = mShader.getVertexUniformLocation("proj_view_mtx");
    mVertexUniformLocation[VERTEX_UNIFORM_M] = mShader.getVertexUniformLocation("model_mtx");
    ...
    mShaderCallback.pObj = this;
    mShaderCallback.pApplyAlphaTestFunc = &MiiShader::ApplyAlphaTestCallback;
    mShaderCallback.pDrawFunc = &MiiShader::DrawCallback;
    mShaderCallback.pSetMatrixFunc = &MiiShader::SetMatrixCallback;
    FFLSetShaderCallback(&mShaderCallback);
}
...
void MiiShader::setViewUniform(const rio::Matrix44f& model_mtx, const rio::BaseMtx34f& view_mtx, const rio::Matrix44f& proj_mtx)
{
    rio::Matrix44f vp;
    vp.setMul(proj_mtx, view_mtx);
    mShader.setUniform(vp, mVertexUniformLocation[VERTEX_UNIFORM_VP], u32(-1));
    mShader.setUniform(model_mtx, mVertexUniformLocation[VERTEX_UNIFORM_M], u32(-1));
}

void MiiShader::applyAlphaTest(bool enable, rio::Graphics::CompareFunc func, f32 ref) const
{
#if RIO_IS_CAFE
...
void MiiShader::setCulling(FFLCullMode mode)
{
...
void MiiShader::Draw(const FFLDrawParam& draw_param)
{
